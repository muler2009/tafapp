...
const TableFilter = ({table, onMonthChange, filter_title}: TableFilterProps) => {
  ...
  return (
    <Div className="relative ml-1">
      <select value={selectedMonth} onChange={handleMonthChange} className={`select-md rounded-sm font-Poppins py-2 px-4 w-full text-[12px] bg-gray-50`} >
        <option value="" className={`font-Poppins px-2 text-[12px] ${filter_title && 'text-[#333] text-opacity-75'}`}> {filter_title} </option>
          {
            [...Array(12)].map((_, i) => (
